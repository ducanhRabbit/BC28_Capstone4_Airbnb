...
export default function SidebarAdmin({}: Props) {
  ...
  return (
    ...
    <Box className='admin-sidebar' sx={{
      transition: 'all 0.2s ease-in-out'
    }}>
      <Box className={`top-sidebar d-flex ${!isCollapsed?'justify-content-between':'justify-content-center'} align-items-center`} sx={{
        px: '20px',
        minHeight: '80px' 
      }}>
      ...
      <Box className='body-section my-3'>
        ...
        {!isCollapsed && <Box className='d-flex justify-content-center align-items-center flex-column mb-3'>
          <Box sx={{
            width: '80px',
            height: '80px',
            overflow: 'hidden',
        ...
        {menuList.map((item,index)=>{
          return (<Box key={item.id} sx={{
            ...
            <NavLink to={item.path} end  className={({ isActive }) => (isActive ? 'active d-block w-100' : 'd-block w-100')}>
              <Box className={`d-flex align-items-center ${!isCollapsed?'':'justify-content-center'} w-100`} sx={{
                padding: '16px 12px',
                gap: '12px',
                overflow: 'hidden',
                color:'#fff'
